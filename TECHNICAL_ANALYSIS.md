# allTranslate 技术实现深度分析

## 文档说明

本文档详细分析 allTranslate 项目中文本和表格检测、处理及大模型翻译的完整技术流程。

**分析日期**: 2025年4月
**项目版本**: allTranslate-main
**核心问题**:
...
allTranslate 采用**非OCR方案**，直接解析PDF内部结构来提取文本，使用深度学习模型进行布局检测，然后将文本块发送给大模型进行翻译，最后保持原布局写回PDF。
...
**allTranslate 完全不使用传统OCR**（如Tesseract、PaddleOCR等），原因如下：
