"""Functions that can be used for the most common use-cases for allTranslate.six"""
...
from allTranslate.converter_docx import convert_to_pdf, is_convertible
...
from allTranslate.converter import TranslateConverter
from allTranslate.doclayout import OnnxModel
from allTranslate.pdfinterp import PDFPageInterpreterEx
    ...
    **kwarg: Any,
    ...
    with tqdm.tqdm(total=total_pages) as progress:
        for pageno, page in enumerate(PDFPage.create_pages(doc)):
            ...
            h, w = box.shape
            # 使用用户配置的跳过类别，如果没有提供则使用默认值
            # 注意：table 不跳过，因为需要特殊处理（使用负值标记）
            # 必须跳过的元素（不显示在界面上）：abandon, isolate_formula, formula_caption
            mandatory_skip = ["abandon", "isolate_formula", "formula_caption"]
            if skip_classes is not None:
                vcls = mandatory_skip + skip_classes
            else:
                vcls = mandatory_skip
            # 转换为小写以实现大小写不敏感匹配
            vcls_lower = [c.lower() for c in vcls]
            for i, d in enumerate(page_layout.boxes):
                # 使用正数标记普通文本区域，负数标记表格区域
                cls_value = i + 2
                if page_layout.names[int(d.cls)].lower() == "table":
                    cls_value = -(i + 2)  # 表格区域用负数标记
            ...
            for i, d in enumerate(page_layout.boxes):
                if page_layout.names[int(d.cls)].lower() in vcls_lower:
                    x0, y0, x1, y1 = d.xyxy.squeeze()
                    x0, y0, x1, y1 = (
                        np.clip(int(x0 - 1), 0, w - 1),
                        np.clip(int(h - y1 - 1), 0, h - 1),
