...
TRANSLATOR_CONFIG_PATH = PROJECT_ROOT / "translator_config.json"
LOCAL_TRANSLATOR_CONFIG_PATH = PROJECT_ROOT / "translator_config.local.json"
SENSITIVE_PLACEHOLDERS = {"", "xx", "***"}
...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self._translator_config_path = TRANSLATOR_CONFIG_PATH
        self._local_translator_config_path = LOCAL_TRANSLATOR_CONFIG_PATH
        self._config_data = {}
    ...
    def _load_config(self):
        ...
        with self._lock:  # 加锁确保线程安全
            ...
            self._merge_translator_config(self._translator_config_path)
            self._merge_translator_config(self._local_translator_config_path)
    ...
    def _save_config(self):
        ...
        with self._lock:  # 加锁确保线程安全
            ...
            cleaned_data.pop("translators", None)
            with self._config_path.open("w", encoding="utf-8") as f:
                json.dump(cleaned_data, f, indent=4, ensure_ascii=False)

    def _merge_translator_config(self, path):
        if not path.exists():
            return
        with path.open("r", encoding="utf-8") as f:
            data = json.load(f)
        for item in data.get("translators", []):
            self._upsert_translator(item.get("name"), item.get("envs", {}))

    def _upsert_translator(self, name, envs):
        if not name:
            return
        translators = self._config_data.setdefault("translators", [])
        for translator in translators:
            if translator.get("name") == name:
                translator_envs = translator.setdefault("envs", {})
                for key, value in envs.items():
                    if value not in SENSITIVE_PLACEHOLDERS or key not in translator_envs:
                        translator_envs[key] = value
                return
        translators.append({"name": name, "envs": copy.deepcopy(envs)})

    def _save_translator_config(self, name, new_translator_envs):
        path = self._local_translator_config_path
        data = {"translators": []}
        if path.exists():
            with path.open("r", encoding="utf-8") as f:
                data = json.load(f)
    ...
    def set_translator_by_name(cls, name, new_translator_envs):
        ...
        with instance._lock:
            ...
            for translator in translators:
                if translator.get("name") == name:
                    translator["envs"] = copy.deepcopy(new_translator_envs)
                    instance._save_translator_config(name, new_translator_envs)
                    return
            translators.append(
