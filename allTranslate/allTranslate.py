...
from allTranslate import __version__, log
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "-v",
        action="version",
        version=f"allTranslate v{__version__}",
    )
    parser.add_argument(
        "--debug",
        "-d",
    ...
    parse_params.add_argument(
        "--mcp", action="store_true", help="Launch allTranslate MCP server in STDIO mode"
    )

    parse_params.add_argument(
        "--sse", action="store_true", help="Launch allTranslate MCP server in SSE mode"
    )
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    from allTranslate.doclayout import ModelInstance, OnnxModel, set_backend
    ...
    if parsed_args.interactive:
        from allTranslate.gui import setup_gui
    ...
    if parsed_args.flask:
        from allTranslate.backend import flask_app

        flask_app.run(port=11008)
        return 0

    if parsed_args.celery:
        from allTranslate.backend import celery_app
    ...
    if parsed_args.mcp:
        ...
        from allTranslate.mcp_server import create_mcp_app, create_starlette_app
...
def yadt_main(parsed_args) -> int:
    ...
    from allTranslate.high_level import download_remote_fonts
