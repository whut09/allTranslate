...
from allTranslate.cache import TranslationCache
...
class OpenAITranslator(BaseTranslator):
    ...
    CustomPrompt = True
    _client_lock = threading.Lock()  # Class-level lock for thread-safe API calls
    ...
    def do_translate(self, text) -> str:
        with self._client_lock:  # Thread-safe API call
...
class ModelScopeTranslator(OpenAITranslator):
    ...
    def __init__(
        ...
        # 映射到 OPENAI_* 键，确保父类初始化成功
        self.envs["OPENAI_BASE_URL"] = base_url
        self.envs["OPENAI_API_KEY"] = api_key
        self.envs["OPENAI_MODEL"] = model
        self.envs.setdefault("OPENAI_STREAM", "true")
        self.envs.setdefault("OPENAI_STOP_TOKENS", "")
        self.envs.setdefault("OPENAI_MAX_TOKENS", -1)
        super().__init__(
            lang_in,
            lang_out,
            model,
...
class OpenAIlikedTranslator(OpenAITranslator):
    ...
    def __init__(
        ...
        self.envs["OPENAI_API_KEY"] = api_key
        self.envs["OPENAI_MODEL"] = model
        self.envs.setdefault("OPENAI_STREAM", self.envs.get("OPENAILIKED_STREAM", "false"))
        self.envs.setdefault("OPENAI_STOP_TOKENS", self.envs.get("OPENAILIKED_STOP_TOKENS", ""))
        self.envs.setdefault("OPENAI_MAX_TOKENS", self.envs.get("OPENAILIKED_MAX_TOKENS", -1))
        super().__init__(
            lang_in,
            lang_out,
            model,
...
class QwenMtTranslator(OpenAITranslator):
    ...
    def __init__(
        ...
        # 映射 ALI_* 到 OPENAI_*，确保父类初始化时能访问到
        self.envs["OPENAI_BASE_URL"] = base_url
        self.envs["OPENAI_API_KEY"] = api_key
        self.envs["OPENAI_MODEL"] = model
        self.envs["OPENAI_STREAM"] = "true"  # 默认值
        self.envs["OPENAI_STOP_TOKENS"] = ""
        self.envs["OPENAI_MAX_TOKENS"] = -1
