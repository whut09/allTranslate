...
from allTranslate.kernel.v2_bridge import request_to_cli_args, request_to_env
...
class PreciseKernel:
    ...
    def ensure_venv(self) -> None:
        ...
        if (
            ...
            raise RuntimeError(
                "allTranslate precise submodule not found. "
                "Run: git submodule update --init allTranslate/kernel/PDFMathTranslate-next.git"
            )
    ...
    def _build_subprocess_env(self, request: TranslateRequest) -> dict[str, str]:
        """Build environment for the subprocess with ALLTRANSLATE_ prefixed vars."""
        env = os.environ.copy()
